...
- [x] `test-files/std/runtime/stack-trace-dump-basic` covers `sDumpStacktrace()` on every platform. The printed
      trace is not portable (frame count, resolved names and offsets all differ per platform), and the test
      runner folds stderr into the compared output, so the test points stderr at a file first and checks the
      dump line by line afterwards, leaving only deterministic text to compare.
