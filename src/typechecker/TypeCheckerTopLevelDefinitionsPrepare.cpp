...
std::any TypeChecker::visitMainFctDefPrepare(MainFctDefNode *node) {
  ...
  node->returnsOnAllControlPaths(&returnsOnAllControlPaths, manIdx);
...
std::any TypeChecker::visitFctDefPrepare(FctDefNode *node) {
  ...
  if (!node->returnsOnAllControlPaths(&doSetPredecessorsUnreachable, manIdx))
...
std::any TypeChecker::visitStructDefPrepare(StructDefNode *node) {
  ...
  // (assignDeferredOpaqueType), the existing type was already set to the opaque struct type. Overwriting it with the
  // identical interned type is expected here.
  const bool overwrite = node->entry->getQualType().is(TY_STRUCT);
...
std::any TypeChecker::visitInterfaceDefPrepare(InterfaceDefNode *node) {
  ...
  assert(node->entry != nullptr);
  // If the entry was implicitly forward-declared because it is referenced across a circular import
  // (assignDeferredOpaqueType), the existing type was already set to the opaque interface type. Overwriting it with the
  // identical interned type is expected here.
  const bool overwrite = node->entry->getQualType().is(TY_INTERFACE);
