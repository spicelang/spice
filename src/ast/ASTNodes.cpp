...
bool SwitchStmtNode::returnsOnAllControlPaths(bool *doSetPredecessorsUnreachable) const {
  const bool allCaseBranchesReturn = std::ranges::all_of(
      caseBranches, [=](const CaseBranchNode *node) { return node->returnsOnAllControlPaths(doSetPredecessorsUnreachable); });
...
bool TernaryExprNode::hasCompileTimeValue() const {
  return condition->hasCompileTimeValue() && trueValue->hasCompileTimeValue() && falseValue->hasCompileTimeValue();
}
...
CompileTimeValue TernaryExprNode::getCompileTimeValue() const {
  ...
  return condition->getCompileTimeValue().boolValue ? trueValue->getCompileTimeValue() : falseValue->getCompileTimeValue();
}

bool LogicalOrExprNode::hasCompileTimeValue() const {
  return std::ranges::all_of(operands, [](const ExprNode *node) { return node->hasCompileTimeValue(); });
}
...
CompileTimeValue BitwiseOrExprNode::getCompileTimeValue() const {
  ...
  for (size_t i = 1; i < operands.size(); i++) {
    ...
    CompileTimeValue opCompileTimeValue = operands.at(i)->getCompileTimeValue();
...
bool EqualityExprNode::hasCompileTimeValue() const { return lhs->hasCompileTimeValue() && rhs->hasCompileTimeValue(); }

CompileTimeValue EqualityExprNode::getCompileTimeValue() const {
  if (children.size() == 1)
    return children.front()->getCompileTimeValue();

  const CompileTimeValue lhsValue = lhs->getCompileTimeValue();
  const CompileTimeValue rhsValue = rhs->getCompileTimeValue();
  if (op == OP_EQUAL)
    return CompileTimeValue{.boolValue = lhsValue.longValue == rhsValue.longValue};
  if (op == OP_NOT_EQUAL)
    return CompileTimeValue{.boolValue = lhsValue.longValue != rhsValue.longValue};
...
bool RelationalExprNode::hasCompileTimeValue() const { return lhs->hasCompileTimeValue() && rhs->hasCompileTimeValue(); }
...
bool ShiftExprNode::hasCompileTimeValue() const { return lhs->hasCompileTimeValue() && rhs->hasCompileTimeValue(); }
...
CompileTimeValue ShiftExprNode::getCompileTimeValue() const {
  ...
  const CompileTimeValue rhsValue = rhs->getCompileTimeValue();
  if (op == OP_SHIFT_LEFT)
    return CompileTimeValue{.longValue = lhsValue.longValue << rhsValue.longValue};
...
void DataTypeNode::setFieldTypeRecursive() { // NOLINT(*-no-recursion)
  ...
  CustomDataTypeNode *customType = baseDataType->customDataType;
