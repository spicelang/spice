...
def render_nav(modules_by_dir: dict[str, list[Module]], out_dir: Path, doc_root: Path) -> str:
    ...
    lines.append(f'  {{ "Standard Library" = [')
...
# Markers delimiting the auto-generated std section inside the mkdocs.yml nav
MKDOCS_NAV_BEGIN = "# >>> BEGIN std library nav (generated by tools/generate_std_docs.py) >>>"
MKDOCS_NAV_END = "# <<< END std library nav <<<"


def build_mkdocs_nav_block(modules_by_dir: dict[str, list[Module]]) -> list[str]:
    """Build the YAML nav lines for the Standard Library section (2-space indentation)."""
    lines = ["  - Standard Library:", "    - Overview: std/index.md"]
    for subdir in sorted(modules_by_dir):
        modules = sorted(modules_by_dir[subdir], key=lambda m: m.stem)
        if subdir:
            lines.append(f"    - {subdir}:")
            for module in modules:
                lines.append(f"      - {module.title}: std/{subdir}/{module.stem}.md")
        else:
            for module in modules:
                lines.append(f"    - {module.title}: std/{module.stem}.md")
    return lines


def update_mkdocs_nav(path: Path, block_lines: list[str]) -> bool:
    """Replace the marker-delimited std nav block in mkdocs.yml. Returns False if markers are absent."""
    if not path.is_file():
        return False
    text = path.read_text(encoding="utf-8")
    lines = text.splitlines()
    begin = next((i for i, ln in enumerate(lines) if ln.strip() == MKDOCS_NAV_BEGIN), None)
    end = next((i for i, ln in enumerate(lines) if ln.strip() == MKDOCS_NAV_END), None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mkdocs", type=Path, default=Path("docs/mkdocs.yml"),
                        help="mkdocs.yml whose std nav block is updated in place (between markers)")
    parser.add_argument("--skip-mkdocs-nav", action="store_true", help="Do not touch the mkdocs.yml nav")
    ...
    if not args.skip_mkdocs_nav:
        if update_mkdocs_nav(args.mkdocs, build_mkdocs_nav_block(modules_by_dir)):
            print(f"Updated std nav in {args.mkdocs}", file=sys.stderr)
        else:
            print(f"warning: nav markers not found in {args.mkdocs}; skipped nav update", file=sys.stderr)
