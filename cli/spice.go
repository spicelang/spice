...
func main() {
	...
	buildCommand := cli.Command{
		...
		Flags: []cli.Flag{
			&cli.BoolFlag{Name: "debug-output", Aliases: []string{"d"}, Usage: "Print compiler output for debugging"},
			&cli.StringFlag{Name: "target", Aliases: []string{"t"}, Usage: "Target triple for the emitted executable (for cross-compiling)"},
			&cli.IntFlag{Name: "opt-level", Aliases: []string{"o"}, Usage: "Adjust optimization level", Value: 2},
	...
	installCommand := cli.Command{
		...
		Flags: []cli.Flag{
			&cli.BoolFlag{Name: "debug-output", Aliases: []string{"d"}, Usage: "Print compiler output for debugging"},
			&cli.IntFlag{Name: "opt-level", Aliases: []string{"o"}, Usage: "Adjust optimization level"},
		},
		Action: func(c *cli.Context) error {
