...
if (SPICE_DEV_COMPILE)
    ...
    set(SPICE_DEV_FLAGS "-ffunction-sections -fdata-sections -march=native")
    # Default: no mold
    set(SPICE_LD_FLAGS "")
    set(SPICE_LD_GC_FLAGS "")
    # Detect mold
    if (NOT APPLE)
        find_program(MOLD_LINKER mold)
        if (MOLD_LINKER)
            message(STATUS "Spice: mold linker found: ${MOLD_LINKER}")
            set(SPICE_LD_FLAGS "-fuse-ld=mold")
            set(SPICE_LD_GC_FLAGS "-Wl,--gc-sections")
        else ()
            message(STATUS "Spice: mold linker not found, using default linker")
        endif ()
    else ()
        message(STATUS "Spice: mold not supported on macOS, using default linker")
    endif ()
    set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} ${SPICE_LD_FLAGS} ${SPICE_DEV_FLAGS}")
    set(CMAKE_CXX_FLAGS_RELWITHDEBINFO "${CMAKE_CXX_FLAGS_RELWITHDEBINFO} ${SPICE_LD_FLAGS} ${SPICE_DEV_FLAGS}")
    set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} ${SPICE_LD_FLAGS} ${SPICE_DEV_FLAGS}")
    set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} ${SPICE_LD_GC_FLAGS}")
else ()
...
# Experimental TPDE compiler backend (fast, -O0-quality codegen; ELF-only, x86_64/aarch64)
option(SPICE_ENABLE_TPDE "Enable the experimental TPDE compiler backend" OFF)
if (SPICE_ENABLE_TPDE)
    message(STATUS "Spice: Experimental TPDE backend is enabled.")
    add_compile_definitions(SPICE_ENABLE_TPDE)
else ()
    message(STATUS "Spice: Experimental TPDE backend is disabled.")
endif ()
