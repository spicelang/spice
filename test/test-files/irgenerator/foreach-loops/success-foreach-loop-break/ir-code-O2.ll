...
define dso_local i32 @main() local_unnamed_addr #0 {
  ...
  %3 = call ptr @_mf__NumberIterator_short__shortref__get(ptr nonnull %shortIterator) #2
  %s.014 = load i16, ptr %3, align 2
  %4 = call i1 @_mf__NumberIterator_short__bool__isValid(ptr nonnull %shortIterator) #2
  br i1 %4, label %foreach.body.L5.lr.ph, label %foreach.exit.L5
...
foreach.body.L5:                                  ; preds = %foreach.body.L5.lr.ph, %foreach.tail.L5
  %s.015 = phi i16 [ %s.014, %foreach.body.L5.lr.ph ], [ %s.0, %foreach.tail.L5 ]
  %5 = sext i16 %s.015 to i32
  %6 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @printf.str.0, i32 %5)
  %7 = and i16 %s.015, 1
  %.not = icmp eq i16 %7, 0
  br i1 %.not, label %if.exit.L7, label %if.then.L7

if.then.L7:                                       ; preds = %foreach.body.L5
  %8 = call %__long__NumberIterator__long_long_long @"_f__void__NumberIterator<long>__range__long_long"(i64 1, i64 2) #2
  %.fca.0.extract = extractvalue %__long__NumberIterator__long_long_long %8, 0
  store i64 %.fca.0.extract, ptr %1, align 8
  %.fca.1.extract = extractvalue %__long__NumberIterator__long_long_long %8, 1
  store i64 %.fca.1.extract, ptr %.fca.1.gep, align 8
  %.fca.2.extract = extractvalue %__long__NumberIterator__long_long_long %8, 2
  store i64 %.fca.2.extract, ptr %.fca.2.gep, align 8
  %9 = call ptr @_mf__NumberIterator_long__longref__get(ptr nonnull %1) #2
  %10 = load i64, ptr %9, align 8
  %11 = call i1 @_mf__NumberIterator_long__bool__isValid(ptr nonnull %1) #2
  br i1 %11, label %foreach.body.L8, label %if.exit.L7

foreach.body.L8:                                  ; preds = %if.then.L7
  %12 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @printf.str.1, i64 %10)
  br label %foreach.exit.L5

if.exit.L7:                                       ; preds = %if.then.L7, %foreach.body.L5
  %13 = call i1 @_mf__NumberIterator_short__bool__isValid(ptr nonnull %shortIterator) #2
  br i1 %13, label %foreach.tail.L5, label %foreach.exit.L5

foreach.tail.L5:                                  ; preds = %if.exit.L7
  %14 = call ptr @_mf__NumberIterator_short__shortref__next(ptr nonnull %shortIterator) #2
  %s.0 = load i16, ptr %14, align 2
  %15 = call i1 @_mf__NumberIterator_short__bool__isValid(ptr nonnull %shortIterator) #2
  br i1 %15, label %foreach.body.L5, label %foreach.exit.L5

foreach.exit.L5:                                  ; preds = %foreach.tail.L5, %if.exit.L7, %0, %foreach.body.L8
  %16 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @printf.str.2)
  ret i32 0
}
...
declare ptr @_mf__NumberIterator_short__shortref__next(ptr) local_unnamed_addr
